...
while True:
    ...
    #######################
    ret, vid_frame = cam.read()

    #frame_flip = cv2.flip(vid_frame, 1) #flip vertically:0 ; flip horizontally:1 ; flip both vert and hor: -1
    frame_flip=vid_frame #for video pass image as is.
    originalHeight=frame_flip.shape[0]
    originalWidth=frame_flip.shape[1]

    
    resized_frame = imresize(frame_flip,(img_height,img_width),preserve_range=True)
    resized_frame=resized_frame.astype(int)    

    
    imgModel = np.array([resized_frame])
    pred=model.predict(imgModel)
    #print(pred)

    for i in pred:
        for j in i:
            if j[1]>0:
                x_min=int(int(j[2])*(originalWidth/img_width))
                y_min=int(int(j[3])*(originalHeight/img_height))
                x_max=int(int(j[4])*(originalWidth/img_width))
                y_max=int(int(j[5])*(originalHeight/img_height))
                classID=int(j[0])
                className=classes[classID]
                cv2.putText(frame_flip,str(className), (x_min,y_min), cv2.FONT_HERSHEY_SIMPLEX,fontScale=0.6, color=(0, 0, 255))
                cv2.rectangle(frame_flip,(x_min,y_min),(x_max,y_max),(120, 120, 120), 2)
    ...
    frame_flip = cv2.putText(frame_flip, fps, (0, 20), cv2.FONT_HERSHEY_SIMPLEX,
                      fontScale=0.8, color=(0, 255, 255))
    frame_flip = cv2.putText(frame_flip, 'Press q to quit', (440, 20), cv2.FONT_HERSHEY_SIMPLEX,
                      fontScale=0.8, color=(0, 0, 255))
    cv2.imshow('Video feed', frame_flip)
    #cv2.waitKey(0) 
